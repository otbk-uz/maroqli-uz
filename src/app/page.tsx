    ...
    <main className="bg-background min-h-screen relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-white/5 rounded-2xl border border-white/5">
              <p className="text-secondary text-sm">Hozirda faol turnirlar mavjud emas.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {tournaments.map((t) => (
                <div key={t.id} className="glass-card overflow-hidden group hover:border-primary/50 transition-all duration-500">
                  <div className="aspect-[16/10] bg-white/5 relative overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4 flex space-x-2">
                      <span className={`${t.status === 'LIVE' ? 'bg-primary animate-pulse' : 'bg-blue-500'} px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg shadow-black/50`}>
                        {t.status}
                      </span>
                    </div>
                  ...
                  <div className="p-8">
                    <span className="text-[10px] font-bold text-primary uppercase tracking-widest mb-2 block">{t.game_name}</span>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                        ...
                        <p className="text-lg font-black text-white">
                          {Number(t.prize_pool) > 0 ? `$${Number(t.prize_pool).toLocaleString()}` : "Bepul"}
                        </p>
                      </div>
                      <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                        <p className="text-[10px] font-bold text-secondary uppercase tracking-widest mb-1">Slots</p>
                        <p className="text-lg font-black text-white">{t.participant_count}/{t.max_participants}</p>
                      </div>
                    </div>
      ...
      <section className="py-20 border-t border-white/5 relative">
        <div className="container mx-auto px-4 md:px-6">
          <div className="flex flex-col md:flex-row items-end justify-between mb-16 gap-6">
            <div>
              <h2 className="text-4xl font-black text-white">So'nggi Yangiliklar</h2>
              <p className="text-secondary text-sm mt-2">Platforma va gaming koinotidagi yangiliklar</p>
            </div>
          ...
          {loading ? (
            ...
            <p className="text-secondary text-sm text-center py-10">Yangiliklar mavjud emas.</p>
          ) : (
        ...
        <div className="container mx-auto px-4 md:px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12 text-center">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-secondary uppercase tracking-widest">Turnirlar soni</p>
            </div>
            <div>
              <p className="text-4xl md:text-6xl font-black text-white mb-2">{stats.users_count}</p>
              <p className="text-xs font-bold text-secondary uppercase tracking-widest">Faol foydalanuvchilar</p>
            </div>
            <div>
              <p className="text-4xl md:text-6xl font-black text-white mb-2">{stats.games_count}</p>
              <p className="text-xs font-bold text-secondary uppercase tracking-widest">Mavjud o'yinlar</p>
            </div>
          </div>
        </div>
      </section>
